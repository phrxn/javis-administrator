...
public class ComputerPersistenceSQLServer implements ComputerPersistence {

  private ConnectionFactory connectionFactory;
  private Text text;

  public ComputerPersistenceSQLServer(ConnectionFactory connectionFactory) {
    this(
        connectionFactory,
        LanguageFactory.getLanguage(LanguagePathToFile.COMPUTER_PERSISTENCE_SQL_SERVER));
  }

  public ComputerPersistenceSQLServer(ConnectionFactory connectionFactory, Text text) {
  ...
  public Computer createComputer(Computer c) throws PersistenceException {
    ...
    String sql =
        "INSERT INTO computers (status_computer, session_type, "
            + "username, hostname, ip, manual, login_date, client_version, details) "
            + "VALUES(?,?,?,?,?,?,?,?,?)";
  ...
  private Computer insertObject(ResultSet rs, Computer computer) throws PersistenceException {
    ...
    // if the database return the id -1, a computer with the same hostname and ip already exist
    if (id == -1) {
      throw new PersistenceException(
          text.getText(
